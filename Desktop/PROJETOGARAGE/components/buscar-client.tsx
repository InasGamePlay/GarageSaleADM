...
export function BuscarClient() {
  ...
  async function handlePhotoChange(file: File | undefined) {
    ...
    try {
      ...
      const rpcMatches = (data ?? []) as Array<
        Omit<Match, "status" | "created_at">
      >;

      // A RPC não retorna status/created_at; busca esses campos por ID.
      let enriched: Match[] = [];
      if (rpcMatches.length > 0) {
        const ids = rpcMatches.map((m) => m.id);
        ...
        enriched = rpcMatches.map((m) => ({
          ...m,
          status: extraMap.get(m.id)?.status ?? "disponivel",
          created_at: extraMap.get(m.id)?.created_at ?? null,
        }));
      }

      // Aplica filtro de status e ordenação por data no client.
      ...
      setStatus(
        sorted.length > 0
          ? { kind: "resultados", matches: sorted }
          : { kind: "nenhum" }
      );
    } catch (error) {
      setStatus({
      ...
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col gap-6 px-4 py-6">
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2 p-4">
                      ...
                      <p className="text-xs text-zinc-400">
                        Cadastrado em: {formatDate(match.created_at)}
                      </p>
                    </div>
                  </Link>
                </li>
        ...
        {status.kind === "nenhum" && (
          <section className="flex flex-col items-center gap-3 rounded-2xl border border-zinc-200 bg-white px-6 py-10 text-center shadow-sm">
            ...
            <p className="text-sm text-zinc-600">
              Não encontramos objetos parecidos com essa foto (ou nenhum
              corresponde aos filtros). Tente novamente com outro ângulo.
            </p>
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
